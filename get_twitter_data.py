...
def analysis_word_rnk(tweets_data):
    ...
    for word,count in analyzer.analyze(' '.join(tweets_data)):
            analyze_count += 1
            output.append(word)
            logger.info(info_str + '{0}　{1}'.format(word,count))
...
def create_wordcloud(text,hashtag,stop_words):
    ...
    wordCloud = WordCloud(background_color="white",font_path=fpath, width=600, height=500, colormap='spring',stopwords=set(stop_words),max_words=30,max_font_size=300,collocations = False,regexp="[\w']+").generate(text)
    plt.figure(figsize=(6,5))
    plt.imshow(wordCloud)
    plt.axis("off")
    filename = './picture/{0:%Y-%m-%d_}'.format(datetime.now()) + hashtag + '_figure.png'
...
stop_words = [ u'てる', u'いる', u'なる', u'れる', u'する', u'ある', u'こと', u'これ', u'さん', u'して', \
         ...
         u'suwananaka',u'(*',u'*)',u'th',u'なか',u'ー',u'/',u'gt',u't',u'co',u'ん',u'わい',u'5',u'://',u'co',u'(',u'RCC']

#１．引数の判定
if not len(sys.argv) == 4:
#if len(sys.argv) == 4:
    logger.info(info_str + '引数の設定に誤りがあります。下記のフォーマットで設定して下さい。' + str(len(sys.argv)))
    logger.info(info_str + 'python get_twitter_data "hashtag" "開始時間" "終了時間"')
    sys.exit()
...
#テスト用
#hashtag = 'アッパレ173'
#start_time = '2019-06-24_' + '23:15:00' +'_JST'
#end_time = '2019-06-24_' + '23:30:00' +'_JST'
#hashtag = 'anju_RCC'
#start_time = '2019-06-20_' + '19:30:00' +'_JST'
#end_time = '2019-06-20_' + '20:00:00' +'_JST'
#hashtag = 'fuwasata'
...
#ログ
logger = LogStraem.get_logger(hashtag)

#３．API認証
logger.info(info_str + 'Twitter API認証を開始します。')
auth = tweepy.OAuthHandler(consumer_key, consumer_secret)
auth.set_access_token(access_token, access_secret)
api = tweepy.API(auth, wait_on_rate_limit = True)
logger.info(info_str + 'Twitter API認証が完了しました。')

#４．ツイート取得
logger.info(info_str + 'Twitter データ取得を開始します。')
tweets_data = []

#検索キーワードの設定
query = '#{0} since:{1} until:{2}'
query = query.format(hashtag,start_time,end_time)
logger.info(info_str + '検索キーワード ' + query)
...
while exit_flag:
    ...
    logger.info(info_str + '取得回数 ' + str(loop_count) + '回目　／　' + '取得件数 ' + str(len(search_results)) + '件')
...
logger.info(info_str + 'Twitter データ取得が完了しました。')

#取得件数が0の場合は終了する
if get_data == 0:
    logger.info(info_str + 'Twitter データ取得が0件のため、スクリプトを終了します。')
    sys.exit()
...
#ワードクラウド生成・保存
logger.info(info_str + 'ワードクラウドの生成を開始します。')
text = ' '.join(analysed_tweets_data)
filename = create_wordcloud(text,hashtag,stop_words)
...
while len(most_word) == 0:
    if not collect.most_common()[word_loop][0] in stop_words:
        ...
        break
    word_loop += 1
logger.info(info_str + 'ワードクラウドの生成が完了しました。')
print(collect.most_common(100))

#結果をツイートする
api.update_with_media(filename = filename,status ='本日最も呟かれたワードは「{0}」でした。 総ツイート数：{1}件 #{2}'.format(most_word,str(get_data),hashtag))
logger.info(info_str + 'ツイートが完了しました。')
